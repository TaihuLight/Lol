...
instance (SHE ctex, Applicative mon,
  ...
  AddLit (PT2CT m'map zqs gad z v ctex mon) (PNoiseCyc h t m zp) where

  addLit_ (PNC a) = PC $ pure $ addPublic_ a

instance (SHE ctex, Applicative mon,
          MulPublicCtx ctex (Cyc2CT m'map zqs (PNoiseCyc h t m zp))) =>
  MulLit (PT2CT m'map zqs gad z v ctex mon) (PNoiseCyc h t m zp) where
...
instance (PT2CTMulCtx m'map h zqs m zp TrivGad ctex t z v mon)
  => Mul (PT2CT m'map zqs TrivGad z v ctex mon) (PNoiseCyc h t m zp) where

  type PreMul (PT2CT m'map zqs TrivGad z v ctex mon) (PNoiseCyc h t m zp) =
    PNoiseCyc (TotalNoiseUnits zqs (h :+: N2)) t m zp

  mul_ = pt2ctMul

instance (PT2CTMulCtx m'map h zqs m zp (BaseBGad 2) ctex t z v mon)
  => Mul (PT2CT m'map zqs (BaseBGad 2) z v ctex mon) (PNoiseCyc h t m zp) where

  type PreMul (PT2CT m'map zqs (BaseBGad 2) z v ctex mon) (PNoiseCyc h t m zp) =
    PNoiseCyc (TotalNoiseUnits zqs (h :+: N2)) t m zp
...
pt2ctMul :: forall m' m m'map zp t zqs h gad ctex z v mon env hin hintzq .
  ...
  => PT2CT m'map zqs gad z v ctex mon env (PNoiseCyc hin t m zp -> PNoiseCyc hin t m zp -> PNoiseCyc h t m zp)
pt2ctMul = PC $ do
...
instance (SHE ctex, Applicative mon,
  ...
  (PNoiseCyc h t m (ZqBasic ('PP '(Prime2, e)) i)) where

  type PreDiv2 (PT2CT m'map zqs gad z v ctex mon)
       (PNoiseCyc h t m (ZqBasic ('PP '(Prime2, e)) i)) =
    PNoiseCyc h t m (ZqBasic ('PP '(Prime2, 'Lol.S e)) i)
...
type PT2CTLinearCtx' ctex mon m'map zqs h t e r s r' s' z zp zq zqin hintzq v gad =
  (SHE ctex, Lambda ctex, Field v, Algebraic v, Fact s',
   ...
   CT s zp (Cyc t s' zq)   ~ Cyc2CT m'map zqs (PNoiseCyc h t s zp),
   -- input ciphertext type: plaintext has one-larger pnoise
   CT r zp (Cyc t r' zqin) ~ Cyc2CT m'map zqs (PNoiseCyc ('S h) t r zp),
...
instance LinearCyc (PT2CT m'map zqs TrivGad z v ctex mon)
         (Linear t) (PNoiseCyc h t) where

  -- EAC: Danger: as far as GHC is concerned, ('S h) is not the same as (h :+: N1)
  type PreLinearCyc (PT2CT m'map zqs TrivGad z v ctex mon) (PNoiseCyc h t) = PNoiseCyc (h :+: N1) t

  type LinearCycCtx (PT2CT m'map zqs TrivGad z v ctex mon) (Linear t) (PNoiseCyc h t) e r s zp =
...
instance LinearCyc (PT2CT m'map zqs (BaseBGad 2) z v ctex mon)
         (Linear t) (PNoiseCyc h t) where

  -- EAC: Danger: as far as GHC is concerned, ('S h) is not the same as (h :+: N1)
  type PreLinearCyc (PT2CT m'map zqs (BaseBGad 2) z v ctex mon) (PNoiseCyc h t) = PNoiseCyc (h :+: N1) t

  type LinearCycCtx (PT2CT m'map zqs (BaseBGad 2) z v ctex mon) (Linear t) (PNoiseCyc h t) e r s zp =
    (PT2CTLinearCtx ctex mon m'map zqs h t e r s (Lookup r m'map) (Lookup s m'map)
...
pt2ctLinearCyc :: forall t zp e r s env expr r' s' zq h zqs m'map gad z v ctex mon .
  (expr ~ PT2CT m'map zqs gad z v ctex mon, s' ~ Lookup s m'map,
   PT2CTLinearCtx ctex mon m'map zqs h t e r s (Lookup r m'map) (Lookup s m'map)
    z zp (PNoise2Zq zqs h) (PNoise2Zq zqs (h :+: N1)) v gad,
   Cyc2CT m'map zqs (PNoiseCyc h t r zp) ~ CT r zp (Cyc t r' zq))
    => Linear t zp e r s -> expr env (PNoiseCyc (h :+: N1) t r zp -> PNoiseCyc h t s zp)
  ...
  return $ lam $
    ...
    (tunnel_ hint $:     -- linear w/ the hint
      (modSwitch_ $: -- then scale (up) to the hint modulus zq'
        (v0 :: ctex _ (Cyc2CT m'map zqs (PNoiseCyc (h :+: N1) t r zp)))))

----- Type families -----

type family Cyc2CT (m'map :: [(Factored, Factored)]) zqs e = cte | cte -> e where

  Cyc2CT m'map zqs (PNoiseCyc h t m zp) =
