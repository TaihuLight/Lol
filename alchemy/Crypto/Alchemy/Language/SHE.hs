...
class Lambda expr => SHE expr where
...
class ErrorRate expr where
  ...
  errorRate_ :: (ErrorRateCtx expr ct z, ct ~ CT m zp (Cyc t m' zq))
             => SK (Cyc t m' z) -> expr e (Arrow expr ct Double)
